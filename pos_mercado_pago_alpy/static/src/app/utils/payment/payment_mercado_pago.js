...
export class PaymentMercadoPago extends PaymentInterface {
    setup(pos, payment_method_id) {
        ...
        this.mp_qr_order_id = null;
        this.mp_qr_ext_ref = null;
    }
    ...
    // ── PaymentInterface overrides (Odoo 19 camelCase) ───────────────────────

    /**
     * @param {string} cid - UUID of the payment line
        ...
        return await new Promise((resolve) => {
            ...
            const pollInterval = setInterval(async () => {
                ...
                try {
                    ...
                    if (
                        statusResp &&
                        ["processed", "finished", "canceled", "failed", "expired", "closed"].includes(statusResp.status)
                    ) {
                        clearInterval(pollInterval);
    ...
    async _handleQrWebhook() {
        ...
        if (!statusResp || !this._isOrderResponseValid(statusResp)) return;
        if (!this._isOrderApproved(statusResp) && !this._isOrderRejected(statusResp)) return;
            ...
            return await new Promise((resolve) => {
                ...
                const s = setInterval(async () => {
                    ...
                    if (retry_cnt >= MAX_RETRY) {
                        clearInterval(s);
                        resolve(showMessageAndResolve(_t("Payment status could not be confirmed"), "error", false));
                    }
                }, RETRY_DELAY);
            });
        }

...
Este módulo permite imprimir los datos completos del cliente en **comandas de cocina**, **ticket de cierre de venta** y **facturas**, facilitando la logística y entrega de pedidos para locales que operan con despacho a domicilio o delivery.
...
- **Ticket de Cierre de Venta (`OrderReceipt`)**:
  - Bloque formateado bajo el encabezado fiscal/comercial, ideal para entregar al repartidor o cliente final.
- **Factura Oficial (PDF y ticket fiscal)**:
  - Integra datos de contacto (teléfono y correo) en el reporte de factura de Odoo (`account.report_invoice_document` y `l10n_ar.report_invoice_document`) cuando la venta proviene de un POS con entrega activa.

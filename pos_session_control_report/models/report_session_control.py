...
class ReportSessionControl(models.AbstractModel):
    ...
    def _get_report_values(self, docids: list, data: dict | None = None) -> dict:
        ...
        env = self.env
        # sale_details contains a 'currency' key which is a plain dict (symbol, position,
        # total_paid, precision). We pass the actual res.currency recordset under 'currency_id'
        # so formatLang receives a proper recordset with the .decimal_places attribute.
